...
- **Photo log (e.g. meal tracking)** — take a photo any time during the day (with an optional note per photo), viewable in-app for the *current day only* (no browsing past days from within the app — that history lives in the Sheet afterward). Design, worked out but not built:
  - **Capture & local storage**: `<input type="file" accept="image/*" capture="environment">` for camera access (works fine in an installed iOS PWA). Store the captured photo + its note in **IndexedDB**, not localStorage — localStorage can't hold binary data and caps out around 5–10MB, nowhere near enough for photos. Resize/compress client-side (canvas) before storing/uploading — full phone-camera resolution is overkill for this.
  - **Today view**: a new tab (or a Home section) reads today's photos+notes straight from IndexedDB and displays them as a simple list/grid. Must work fully offline — a photo taken with no connection still needs to show up immediately in this view.
  - **Upload timing**: tie the Drive upload + Sheets write to the *existing* end-of-day "Save & log day" flow (`EveningReviewModal` → `saveDailyLog`) rather than a separate timer/job — that's the same moment the day's row already gets written, so it's the natural, already-existing sync point. On confirmed success, clear that day's IndexedDB photo cache.
  - **Storage**: upload each photo to Google Drive via the existing `drive.file` OAuth scope (already granted — no new permission needed). This scope only grants access to files the app itself creates, which is exactly right here.
  - **Sheets display**: add columns to that day's log row — a note-text column, plus an `=IMAGE(url)` formula cell per photo so the photo is visible directly in the sheet (not just a link). Rebecca has explicitly accepted that `=IMAGE()` can sometimes fail to render and that's fine, since the file is still backed up in Drive regardless.
  - **Known constraint on `=IMAGE()` reliability**: it depends on Sheets actually being able to fetch the photo's URL. A `drive.file`-uploaded file is private by default, owned by Rebecca's account — whether `=IMAGE()` renders reliably depends on the exact URL format used (e.g. a Drive `uc?export=view&id=...`-style URL vs. requiring "anyone with the link" sharing). This is the specific thing to get right/tune when actually implementing, since it directly determines how often the image renders vs. shows a broken-image icon.
  - **Offline handling**: if a photo is taken offline, it must still appear in the today-view (from IndexedDB) and needs to queue for Drive/Sheets upload once back online — similar in spirit to the existing "couldn't reach Google Sheets, saved locally, retry" handling in the evening review save flow.
  - **Explicitly out of scope per Rebecca**: no in-app gallery/browsing of past days' photos — once uploaded and the local cache clears, viewing history is Sheets-only.
